...
/// <summary>
/// Parses strings to enum values using case-insensitive name, numeric, and dotted-alias matching.
/// </summary>
/// <typeparam name="Enumtype">The enum type to parse.</typeparam>
public sealed class EnumParser<Enumtype> where Enumtype : Enum
{
    /// <summary>
    /// Gets the dictionary mapping string aliases to their corresponding enum values.
    /// </summary>
    public IReadOnlyDictionary<string, Enumtype> EnumValuesDictionary { get; } = BuildDictionary();

    private static ReadOnlyDictionary<string, Enumtype> BuildDictionary()
    {
        var dict = new Dictionary<string, Enumtype>(StringComparer.OrdinalIgnoreCase);
        foreach (var (key, value) in Enum.GetValues(typeof(Enumtype)).Cast<Enumtype>().SelectMany(GetAliases))
        {
            dict.TryAdd(key, value);
        }
        return new ReadOnlyDictionary<string, Enumtype>(dict);
    }

    /// <summary>
    /// Parses a string to the corresponding enum value, or returns <c>default</c> if not found.
    /// </summary>
