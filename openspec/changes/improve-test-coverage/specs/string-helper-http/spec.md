...
`HttpGetStringAsync(url, timeout?, ct)` SHALL perform a GET request and return
the response body as a `string`. Shares all SSRF validation.
...
body into `T`. SHALL throw `InvalidOperationException` when the result is null.
...
- **WHEN** the response body cannot be deserialized to `T`
...
- **THEN** the POST body is null/empty and the server response is returned
