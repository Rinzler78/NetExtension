...
`DeserializeObjectFromFile` validates the file path before reading:

- **Allowed extensions**: `.json`, `.xml`, `.csv`, `.txt`, `.yaml`, `.yml`, `.toml`, `.ini`, `.conf`
- **Blocked**: directory traversal (`../`), home expansion (`~`), all other extensions
