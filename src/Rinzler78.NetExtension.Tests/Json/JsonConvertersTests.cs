...
public class JsonConvertersTests
{
    ...
    public void DoubleConverter_ShouldReadWriteAndValidate()
    {
        ...
        // Comma-as-decimal-separator should be stripped and parsed as integer (e.g. "12,5" → 125)
        var commaReader = new JsonTextReader(new StringReader("\"12,5\""));
        commaReader.Read();
        DoubleConverter.Singleton.ReadJson(commaReader, typeof(double), null, Newtonsoft.Json.JsonSerializer.Create()).Should().Be(125d);
