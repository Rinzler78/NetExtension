...
### Retry Logic
```csharp
public static async Task<ProcessOutputs> RunWithRetryAsync(
    string command, 
    string arguments,
    int maxRetries = 3,
    TimeSpan delay = default)
{
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++)
    {
        try
        {
            var result = await SafeRunCommandAsync(command, arguments);
            if (result.Success)
            {
    ...
    throw new InvalidOperationException($"Command failed after {maxRetries} attempts");
}
```
