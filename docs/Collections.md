...
public static class CollectionHelper
{
    ...
    // Synchronise une collection : supprime les éléments absents, ajoute les nouveaux
    public static void Set<T>(this ICollection<T> collection, IEnumerable<T> newItems)
    {
        var newList = newItems.ToList();
        var toRemove = collection.Except(newList).ToList();
        var toAdd = newList.Except(collection).ToList();
...
### Collection Synchronization with `Set<T>`
```csharp
// Synchronise la collection : supprime ce qui n'est plus là, ajoute ce qui manque
var current = new ObservableCollection<string> { "a", "b", "c" };
current.Set(new[] { "b", "c", "d" });
// current → ["b", "c", "d"]

// Cas d'usage : rafraîchir une liste de résultats sans recréer la collection
var items = new List<string> { "old1", "keep", "old2" };
items.Set(new[] { "keep", "new1", "new2" });
// items → ["keep", "new1", "new2"]
```
