...
public class ProcessHelperDeadlockTests
{
    ...
    public async Task WaitProcessOutputsAsync_LargeOutput_DoesNotDeadlock()
    {
        ...
        // Assert — Windows cmd uses spaces ("stdout line 1"), Unix bash uses underscores ("stdout_line_1")
        outputs.Should().NotBeNull();
        var expectedStdOut = OperatingSystem.IsWindows() ? "stdout line 1" : "stdout_line_1";
        var expectedStdErr = OperatingSystem.IsWindows() ? "stderr line 1" : "stderr_line_1";
        outputs.StdOut.Should().Contain(expectedStdOut);
        outputs.StdErr.Should().Contain(expectedStdErr);
    }
}
